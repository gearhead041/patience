...
export function shuffle<T>(array: T[], rng = Math.random) {
...
export function shareCards(world: World, tableaus: Entity[], deck: Entity[],stock:Entity) {
  let deckIndex = 0;
  ...
  createCards(deck: Entity[]) {
    for (const suit of suits) {
      for (const rank of ranks) {
        var card = this.createEntity();
        this.cards.set(card, { suit, rank });
        this.faceUp.set(card, { value: false });
        deck.push(card);
      }
    }
  }
